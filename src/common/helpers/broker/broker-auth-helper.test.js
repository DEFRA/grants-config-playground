...
const CONFIG_AUTH_TOKEN = 'configBroker.auth.token'
const CONFIG_ENCRYPTION_KEY = 'configBroker.auth.encryptionKey'
...
const mockConfigGet = vi.fn((key) => {
  if (key === 'configBroker.auth.token') {
    return 'default-token'
  }
  if (key === 'configBroker.auth.encryptionKey') {
...
async function importConfigBrokerHelper() {
  return await import('./broker-auth-helper.js')
}

function setupMockConfig(value, encryptionKey = TEST_ENCRYPTION_KEY) {
  const configValues = {
    [CONFIG_AUTH_TOKEN]: value,
...
describe('Broker Auth Helper', () => {
  ...
  describe('Token Encryption', () => {
    ...
    it('should encrypt token when encryption key is configured', async () => {
      const configValues = {
        [CONFIG_AUTH_TOKEN]: MOCK_TOKENS.DEFAULT,
