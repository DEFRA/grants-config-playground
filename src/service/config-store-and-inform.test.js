...
vi.mock('#/common/helpers/broker/broker-auth-helper.js', () => ({
...
describe('storeConfigVersionAndInformBroker', () => {
  ...
  it('should exit early when configurations directory does not exist or not a directory', async () => {
    ...
    await expect(storeConfigVersionAndInformBroker(mockLogger)).resolves.not.toThrow()
  ...
  it('should store config and inform config broker', async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith("uploading 'grant-1/1.2.3/main.json' to S3")
    expect(mockLogger.info).toHaveBeenCalledWith("successfully uploaded '1' files across '1' configs")
  ...
  it('should call config broker for each config (subdirectory) found in configurations folder', async () => {
    ...
    readdirSync
      ...
      .mockReturnValueOnce([
        { name: 'main.json', isFile: () => true, parentPath: 'configurations/grant-1' },
        { name: 'submain.json', isFile: () => true, parentPath: 'configurations/grant-1' }
      ])
      .mockReturnValueOnce([{ name: 'main.json', isFile: () => true, parentPath: 'configurations/grant-2' }])

    await storeConfigVersionAndInformBroker(mockLogger)

    expect(mockLogger.info).toHaveBeenCalledWith("uploading 'grant-1/1.2.3/main.json' to S3")
    expect(mockLogger.info).toHaveBeenCalledWith("uploading 'grant-1/1.2.3/submain.json' to S3")
    expect(mockLogger.info).toHaveBeenCalledWith("uploading 'grant-2/1.2.3/main.json' to S3")
    expect(mockLogger.info).toHaveBeenCalledWith("successfully uploaded '3' files across '2' configs")
    expect(mockLogger.info).toHaveBeenCalledWith(
      "successfully notified the config broker about 'grant-1' at version '1.2.3'"
    )
    expect(mockLogger.info).toHaveBeenCalledWith(
