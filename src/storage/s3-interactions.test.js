  ...
  describe('listFiles', () => {
    it('should list files with a given prefix', async () => {
      ...
      const mockListResponse = {
        Contents: [{ Key: 'test-prefix/file1.txt' }, { Key: 'test-prefix/file2.txt' }]
      }
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(`Found 2 files using prefix ${prefix}`)
    })
    ...
    it('should return an empty array if no files are found', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(`Found 0 files using prefix ${prefix}`)
    })
